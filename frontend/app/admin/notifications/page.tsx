    ...
    <DashboardLayout>
      <div className="p-6 md:p-8 max-w-2xl mx-auto">
        ...
        {loading ? <LoadingState /> : notifs.length === 0 ? (
          <EmptyState icon={<Bell size={40} />} title="No notifications" />
        ) : (
          <div className="space-y-2">
            {notifs.map(n => (
              <button key={n.id} onClick={() => markRead(n.id)} className="w-full text-left">
                <Card className={`transition-all ${!n.is_read ? 'border-indigo-200 bg-indigo-50/50' : ''}`}>
                  <div className="flex items-start gap-3">
                    <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${!n.is_read ? 'bg-indigo-500' : 'bg-gray-200'}`} />
